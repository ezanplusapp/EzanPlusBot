...
def create_paper_background(w: int, h: int, center_rgb: tuple, outer_rgb: tuple) -> Image.Image:
    """Creates a velvety matte paper background with gentle radial vignette."""
    y, x = np.ogrid[:h, :w]
    cx, cy = w / 2, h * 0.45
...
def ciz_acik_pastel_kuran_sozlugu(
    ...
    tone_style: str = "pudra_kirmizi", # "pudra_kirmizi", "gul_kurusu_acik", "kiremit_blush", "blush_paper_dark_text"
    dosya_adi: str = "acik_pastel.png"
):
    w = 1080
    ...
    # Tone definitions
    if tone_style == "pudra_kirmizi":
        # Mid-light Powder Crimson Pastel (Soft, clear, velvety)
        center_rgb = (168, 68, 74)   # #A8444A
        outer_rgb = (138, 48, 54)    # #8A3036
        dark_text = False
        C_HERO = "#FFFFFF"
        C_ARABIC = "#FFF8ED"
        C_ACCENT = "#FDE6BA"         # Soft pale gold
        C_DIVIDER = "#C8757C"
        C_BODY = "#FFFDF9"
        C_QUOTE = "#FDE6BA"
        C_ITALIC = "#FFF6EA"
    ...
    elif tone_style == "gul_kurusu_acik":
        # Lighter Dusty Rose Crimson (Very soft & muted pastel red)
        center_rgb = (182, 86, 92)   # #B6565C
        outer_rgb = (152, 64, 70)    # #984046
        dark_text = False
        C_HERO = "#FFFFFF"
        C_ARABIC = "#FFFDF5"
        C_ACCENT = "#FFEDB8"
    ...
    elif tone_style == "kiremit_blush":
        # Warm Terracotta Blush Pastel
        center_rgb = (186, 96, 88)   # #BA6058
        outer_rgb = (156, 74, 66)    # #9C4A42
        dark_text = False
        C_HERO = "#FFFFFF"
        C_ARABIC = "#FFF8EE"
        C_ACCENT = "#FFEEC4"
    ...
    else: # blush_paper_dark_text
        # Very light pastel rose paper with deep noble burgundy ink
        center_rgb = (242, 218, 216) # #F2DAD8 (soft pastel blush red paper)
        outer_rgb = (226, 194, 192)  # #E2C2C0
        dark_text = True
        C_HERO = "#4A141A"           # Deep noble burgundy
        C_ARABIC = "#380D12"         # Deep calligraphy ink
        C_ACCENT = "#8C2A32"         # Rich crimson accent
        C_DIVIDER = "#D2A2A6"
        C_BODY = "#2E1619"           # Deep dark warm ink
        C_QUOTE = "#8C2A32"
        C_ITALIC = "#3D1A1E"
        C_REF = "#8C2A32"
        C_SEAL = "#8C2A32"
...
if __name__ == "__main__":
    # 1. Pudra Kırmızı (Orta Açıklıkta Kadife Kırmızı Pastel)
    ciz_acik_pastel_kuran_sozlugu(tone_style="pudra_kirmizi", format_tipi="9:16", dosya_adi="acik_pudra_kirmizi_9_16.png")
    ciz_acik_pastel_kuran_sozlugu(tone_style="pudra_kirmizi", format_tipi="4:5", dosya_adi="acik_pudra_kirmizi_4_5.png")

    # 2. Açık Gül Kurusu (Daha Açık & Ferah Pastel Kırmızı)
    ciz_acik_pastel_kuran_sozlugu(tone_style="gul_kurusu_acik", format_tipi="9:16", dosya_adi="acik_gul_kurusu_9_16.png")
    ciz_acik_pastel_kuran_sozlugu(tone_style="gul_kurusu_acik", format_tipi="4:5", dosya_adi="acik_gul_kurusu_4_5.png")

    # 3. Kiremit Blush (Sıcak & Açık Toprak Kırmızısı Pastel)
    ciz_acik_pastel_kuran_sozlugu(tone_style="kiremit_blush", format_tipi="9:16", dosya_adi="acik_kiremit_blush_9_16.png")
    ciz_acik_pastel_kuran_sozlugu(tone_style="kiremit_blush", format_tipi="4:5", dosya_adi="acik_kiremit_blush_4_5.png")

    # 4. Pudra Allık / Blush Red Paper (Koyu Bordo Yazılı Pastel Kağıt)
    ciz_acik_pastel_kuran_sozlugu(tone_style="blush_paper_dark_text", format_tipi="9:16", dosya_adi="acik_blush_koyu_yazili_9_16.png")
    ciz_acik_pastel_kuran_sozlugu(tone_style="blush_paper_dark_text", format_tipi="4:5", dosya_adi="acik_blush_koyu_yazili_4_5.png")
