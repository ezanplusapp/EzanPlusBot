...
def render(cikti_yolu, sag_marka_stili="lora_bold", rozet_genislik=320):
    w, h = 1080, 1350
    kirmizi_ton = "#E2585D"  # Kullanıcının onayladığı İpeksi Pastel Kırmızı
    ...
    # 1. YENİ ÜST HEADER DÜZENİ:
    # Sol: Logo (Ezan Plus ikonu)
    # Orta: Büyütülmüş & Ortalanmış "HADİS-İ ŞERİF" Rozeti
    # Sağ: "Ezan Plus" Marka Yazısı
    sol_x = kx1 + 44
    cur_y = ky1 + 36
    header_h = 58
    mid_header_y = cur_y + header_h // 2

    # A) EN SOLDA LOGO (58x58 px, altın çerçevesiz temiz app ikonu)
    logo_yolu = Path("assets/icons/logo.png")
    logo_size = 58
    if logo_yolu.exists():
        logo = Image.open(logo_yolu).convert("RGBA").resize((logo_size, logo_size), Image.Resampling.LANCZOS)
    ...
    # B) EN SAĞDA "EZAN PLUS"
    if sag_marka_stili == "lora_bold":
        ...
        mh = m_bb[3] - m_bb[1]
        mx = kx2 - 44 - mw
        my = mid_header_y - mh // 2 - m_bb[1] - 4
        draw.text((mx, my), m_txt, font=font_marka, fill="#1C1917")
        font_sub = font_al(FONT_UI, 12, agirlik=600)
        s_txt = "RESMÎ YAYIN"
        s_bb = draw.textbbox((0, 0), s_txt, font=font_sub)
        sw = s_bb[2] - s_bb[0]
        draw.text((kx2 - 44 - sw, my + mh + 5), s_txt, font=font_sub, fill="#8C7A6B")
    elif sag_marka_stili == "lora_sade":
        font_marka = font_al(FONT_GOVDE, 32, agirlik=700)
        m_txt = "Ezan Plus"
        m_bb = draw.textbbox((0, 0), m_txt, font=font_marka)
        mw = m_bb[2] - m_bb[0]
        mh = m_bb[3] - m_bb[1]
        mx = kx2 - 44 - mw
        my = mid_header_y - mh // 2 - m_bb[1]
        draw.text((mx, my), m_txt, font=font_marka, fill="#1C1917")
    else:  # manrope_bold
        font_marka = font_al(FONT_UI, 24, agirlik=800)
        m_txt = "EZAN PLUS"
        m_bb = draw.textbbox((0, 0), m_txt, font=font_marka)
        mw = m_bb[2] - m_bb[0]
        mh = m_bb[3] - m_bb[1]
    ...
    # C) TAM ORTADA BÜYÜTÜLMÜŞ & ORTALANMIŞ "HADİS-İ ŞERİF" ROZETİ
    rozet_w = rozet_genislik
    rozet_h = 50
    rozet_x = (w - rozet_w) // 2
    rozet_y = mid_header_y - rozet_h // 2

    # Rozet gövdesi: Pastel soft kırmızı + altın kenarlık
    yuvarlak_kose_ciz(draw, (rozet_x, rozet_y, rozet_x + rozet_w, rozet_y + rozet_h), radius=16, dolgu=kirmizi_ton, kenarlik="#C29B38", kenarlik_kalinlik=2)
    draw.rounded_rectangle([rozet_x + 4, rozet_y + 4, rozet_x + rozet_w - 4, rozet_y + rozet_h - 4], radius=12, outline=(255, 235, 180, 90), width=1)

    # Rozet Metni
    font_rozet = font_al(FONT_UI, 20, agirlik=800)
    r_txt = "HADİS-İ ŞERİF"
    ...
    draw.text((rx, ry + 1), r_txt, font=font_rozet, fill=(70, 10, 15, 110))
...
if __name__ == "__main__":
    render("/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v6_header_yeni_lora.png", sag_marka_stili="lora_sade", rozet_genislik=320)
    render("/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v6_header_yeni_detayli.png", sag_marka_stili="lora_bold", rozet_genislik=340)
    render("/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v6_header_yeni_manrope.png", sag_marka_stili="manrope_bold", rozet_genislik=320)
