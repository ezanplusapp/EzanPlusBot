...
def render_v7(
    cikti_yolu: str,
    layout_tarzi: str = "simetrik_3lu",  # 'simetrik_3lu', 'tam_logo_sagda_orta_baslik', 'tam_logo_sagda_sol_baslik'
    marka_font_tipi: str = "lora",        # 'lora', 'manrope'
):
    w, h = 1080, 1350
    kirmizi_ton = "#E2585D"  # Seçenek 3 İpeksi Pastel Kırmızı
    ...
    if layout_tarzi == "simetrik_3lu":
        # 1. SOLDA: LOGO İKONU (58x58 px, altın çerçevesiz temiz app ikonu)
        logo_size = 58
        if logo_yolu.exists():
            logo = Image.open(logo_yolu).convert("RGBA").resize((logo_size, logo_size), Image.Resampling.LANCZOS)
            mask = Image.new("L", (logo_size, logo_size), 0)
        ...
        # 3. ORTADA: BÜYÜTÜLMÜŞ & GOLD ÇERÇEVESİZ "HADİS-İ ŞERİF" ROZETİ
        rozet_w = 380
        rozet_h = 54
        rozet_x = (w - rozet_w) // 2
        rozet_y = mid_header_y - rozet_h // 2

        # GOLD ÇERÇEVE YOK! Saf pastel soft kırmızı, pürüzsüz yuvarlatılmış rozet
        yuvarlak_kose_ciz(draw, (rozet_x, rozet_y, rozet_x + rozet_w, rozet_y + rozet_h), radius=18, dolgu=kirmizi_ton)

        font_rozet = font_al(FONT_UI, 22, agirlik=800)
        r_txt = "HADİS-İ ŞERİF"
    ...
    elif layout_tarzi == "tam_logo_sagda_orta_baslik":
        # TAM LOGO BLOĞU SAĞDA ([LOGO] + Ezan Plus / Rehber)
        logo_size = 58
        font_sub = font_al(FONT_UI, 13, agirlik=600)
        s_txt = "SAHİH HADİS-İ ŞERİF REHBERİ"
        s_bb = draw.textbbox((0, 0), s_txt, font=font_sub)
        ...
        text_block_w = max(w_sub, w_marka)
        total_brand_w = logo_size + 16 + text_block_w
        brand_start_x = sag_x - total_brand_w
        ...
        if logo_yolu.exists():
            ...
            mask = Image.new("L", (logo_size, logo_size), 0)
            ImageDraw.Draw(mask).rounded_rectangle([0, 0, logo_size, logo_size], radius=15, fill=255)
            im.paste(logo, (brand_start_x, mid_header_y - logo_size // 2), mask)

        # Metinler
        tx = brand_start_x + logo_size + 16
        start_y = mid_header_y - (h_marka + 4 + (s_bb[3] - s_bb[1])) // 2
        draw.text((tx, start_y - m_bb[1]), m_txt, font=font_marka, fill="#1C1917")
        draw.text((tx, start_y + h_marka + 8 - s_bb[1]), s_txt, font=font_sub, fill="#8C7A6B")

        # Ortada Büyük Rozet (Gold çerçevesiz)
        rozet_w = 340
        rozet_h = 54
        rozet_x = (brand_start_x - 30 - rozet_w) // 2 + 30 # Sol ile logo arası denge
        rozet_y = mid_header_y - rozet_h // 2
        yuvarlak_kose_ciz(draw, (rozet_x, rozet_y, rozet_x + rozet_w, rozet_y + rozet_h), radius=18, dolgu=kirmizi_ton)
    ...
    else:  # tam_logo_sagda_sol_baslik
        # SOLDA: BÜYÜK ROZET (Gold çerçevesiz)
        rozet_w = 360
        rozet_h = 54
        rozet_x = sol_x
        rozet_y = mid_header_y - rozet_h // 2
        ...
        # SAĞDA: TAM LOGO ([LOGO] + Ezan Plus / Rehber)
        logo_size = 58
        font_sub = font_al(FONT_UI, 13, agirlik=600)
        s_txt = "SAHİH HADİS-İ ŞERİF REHBERİ"
        s_bb = draw.textbbox((0, 0), s_txt, font=font_sub)
...
if __name__ == "__main__":
    # Varyasyon 1: Sol Logo, Orta Büyük Başlık (Gold yok), Sağ Ezan Plus + Rehber (Aynı Genişlikte)
    render_v7(
        "/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v7_simetrik_3lu_lora.png",
        layout_tarzi="simetrik_3lu",
        marka_font_tipi="lora"
    )

    # Varyasyon 2: Sol Logo, Orta Büyük Başlık (Gold yok), Sağ EZAN PLUS Caps + Rehber (Aynı Genişlikte)
    render_v7(
        "/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v7_simetrik_3lu_caps.png",
        layout_tarzi="simetrik_3lu",
        marka_font_tipi="manrope"
    )

    # Varyasyon 3: Sağda Tam Logo ([Logo] + Ezan Plus / Rehber), Solda/Ortada Büyük Başlık
    render_v7(
        "/Users/macbook/.gemini/antigravity/brain/6b30e296-9caa-49e5-be7a-db124c169b93/hadis_v7_tam_logo_sagda.png",
        layout_tarzi="tam_logo_sagda_sol_baslik",
        marka_font_tipi="lora"
    )
