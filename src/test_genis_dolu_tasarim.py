...
CIKTI = KOK_DIZIN / "data" / "cikti" / "test_ankebut64_genis.png"
...
# Ana Kart (kx1 = 56, kx2 = W - 56 -> 968 px genişlik, içerik alanı 880 px!)
kx1 = 56
ky1 = 390
kx2 = W - 56
ky2 = 1630
...
# DAHA BÜYÜK VE KARTI SAĞA-SOLA TAM YAYAN ÖLÇÜLER:
pt_ar = 74
pt_okunus = 38
pt_meal = 52
ar_satir_h = 100
...
# Arapça Çiz (Genişletilmiş kelime arası boşluk: 22px)
aktif_idx = 3  # "الدنيا"
for words, start_idx in ar_gruplar:
    ...
    print(f"Arapça Satır Genişliği: {toplam_w}px / Kart İç: {kart_ic_w}px (Kalan yan boşluk: {(kart_ic_w - toplam_w)//2}px)")

    cur_x = (W + toplam_w) // 2
    for w_idx, gorsel_w, width in w_list:
        cur_x -= width
        if w_idx == aktif_idx:
            yuvarlak_kose_ciz(draw, (cur_x - 12, c_y + 2, cur_x + width + 12, c_y + pt_ar + 16), radius=14, dolgu="#FEF3C7")
...
for words, start_idx in tr_gruplar:
    ...
    for w_idx, w_str, width in w_list:
        if w_idx == aktif_idx:
            yuvarlak_kose_ciz(draw, (cur_x - 8, c_y - 2, cur_x + width + 8, c_y + pt_okunus + 10), radius=10, dolgu="#FEF3C7")
...
print(f"Genişletilmiş Meal Bitiş Y: {c_y}, Tefekkür Başlangıç Y: {tef_y1}, Kalan Boşluk: {tef_y1 - c_y}px")
