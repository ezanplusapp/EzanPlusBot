...
* **Kur'an Tilaveti Meal Alanı Kırmızı Degrade Keten Bandı & Metin Odaklı Dinamik Ölçekleme:** Ayet videolarında meal alanı, ferah ve canlı Logo Kırmızısı (`#CE343A` / `(206, 52, 58)` merkez ➔ `#A01E24` / `(160, 30, 36)` dış) Cosine-yumuşatmalı keten zemin bandı ve Saf Beyaz (`#FFFDF9`) / Bold (`#FFFFFF`) tipografiyle işlenir. Kırmızı bandın katı bölgesi (`solid_top` .. `solid_bottom`), mealin kaç satır olduğuna (`meal_blok_h`) göre dinamik olarak büyüyüp küçülür; metin satırları istisnasız %100 katı kırmızı alanın içinde kalır ve degrade geçişine asla taşmaz. Kalan dikey alana göre dinamik auto-fit (`min_gerekli_pay = 105`) ile punto dengelenir; bant ile tefekkür çizgisi arasına altın odak elması yerleştirilir.
